...
def eduedit(request,id):
    ...
    if request.method == "POST":
        ...
        data = Education(qualification=qualification, institution=institution, board=board, start_yr=start_yr, end_yr=end_yr, cgpa=cgpa, percent=percent, person=person)
        edit.update(qualification=qualification, institution=institution, board=board, start_yr=start_yr, end_yr=end_yr, cgpa=cgpa, percent=percent, person=person)
...
def editwork(request,id):
    ...
    if request.method == "POST":
        ...
        if edit_form.is_valid():
            ...
            details = cd.get('details')
            data = Experience(type = type,company = company,role = role, join_dt = join_dt, left_dt = left_dt, details = details, person = person)
            edit.update(type = type,company = company,role = role, join_dt = join_dt, left_dt = left_dt, details = details, person = person)
...
def editpro(request,id):
    ...
    if request.method == "POST":
        ...
        if edit_form.is_valid():
            ...
            data = Projects(project = project, start_dt = start_dt, end_dt = end_dt,running = running, project_link = project_link, person = person)
            edit.update(project = project, start_dt = start_dt, end_dt = end_dt,running = running, project_link = project_link, person = person)
    else:
